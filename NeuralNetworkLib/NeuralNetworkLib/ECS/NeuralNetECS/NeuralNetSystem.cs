...
public sealed class NeuralNetSystem : ECSSystem
{
    ...
    protected override void PreExecute(float deltaTime)
    {
        neuralNetworkComponents =
            new ConcurrentDictionary<uint, NeuralNetComponent>(ECSManager.GetComponents<NeuralNetComponent>());
        outputComponents = 
            new ConcurrentDictionary<uint, OutputComponent>(ECSManager.GetComponents<OutputComponent>());
        inputComponents = 
            new ConcurrentDictionary<uint, InputComponent>(ECSManager.GetComponents<InputComponent>());
        brainAmountComponents =
            new ConcurrentDictionary<uint, BrainAmountComponent>(ECSManager.GetComponents<BrainAmountComponent>());

        IEnumerable<uint> entities = ECSManager.GetEntitiesWithComponentTypes(typeof(NeuralNetComponent),
            typeof(OutputComponent), typeof(InputComponent));
